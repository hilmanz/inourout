...
class loginHelper {
	...
	function goLogin($web=false){
		...
		//check password and phonumber , each field must be same of input value (higher security)
 		if($mopfirst){
			...
			if(!$rs){
				$result['result'] = false;
				$result['message'] = " You cannot access this dashboard ";
				return $result;
			}
			if($rs['n_status']!=1) {
				$result['result'] = false;
	...
	*/
	...
	function getPagesStat($user_id=null,$leader_id=null){
		...
		$sql = "
		SELECT pages.name, pages.id ,pages.type,pages.description ,pages.img,pages.ownerid ,pagetype.name pagetypename, pages.brandid, pages.areaid, pages.brandsubid,pages.city
		FROM my_pages pages
