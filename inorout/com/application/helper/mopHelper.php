...
class mopHelper {
	...
	function checklogin(){
			...
			$pwd= $this->apps->_p('password');
			$sessionid = $this->mopClient->CheckLogin($username,$pwd);
			return $sessionid;
	}
	
	function mop($sessionid=false){
			global $CONFIG; 
			
			// if(	isset ($_SESSION['MOP_SESSION'])) return $_SESSION['MOP_SESSION'];
				
				if(!$sessionid) $sessionid = $_REQUEST['id'];
				$session_mop= $this->mopClient->checkReferral($sessionid);
	...
	function insert_data_from_mop($profile){
		...
		$brandpref = $pro['Brand1_ID'];
		$brandpref2 = $pro['Brand2_ID'];
		$AgeVerifiedDate = date("Y-m-d",strtotime($pro['AgeVerifiedDate']));
		$datetimesexsiting = "2014-04-01";
		$usertype = 0;
		if($AgeVerifiedDate<=$datetimesexsiting)$usertype = 2;
		// if(is_array($brandpref)) $brandpref = implode("_",$brandpref);
		
		$avatar = ( strtolower($pro['Gender']) == 'm' )? 'default.jpg' : 'default.jpg';
		

		$n_status = ($pro['AVType'] == 1 || $pro['AVType'] == 3||  $pro['AVType'] == 2|| $pro['AVType'] == 6)? 1 : 0;
		...
		// $this->logger->log('mop data :'.json_encode($pro));
		if(empty($pro['LastName']))$pro['LastName'] = "";
		$sql = "INSERT INTO 
				social_member (registerid,name,email,register_date,username,city,sex,birthday,img,last_name,StreetName,phone_number,last_login,n_status,login_count,Brand1_ID,Brand2_ID)
				VALUES ('".$pro['RegistrationID']."','".$pro['FirstName']."','".$pro['Email']."',NOW(),'".$pro['Login']."','".$pro['CityID']."','".$pro['Gender']."','".$birthday."','{$avatar}','".$pro['LastName']."','".$streetName."','".$mobilePhone."',$last_login,'$n_status','1','{$brandpref}','{$brandpref2}')
	...
	function update_data_from_mop($profile){
		...
		if( intval($rs['n_status']) == 0 && ($pro['AVType'] == 1 || $pro['AVType'] == 3||  $pro['AVType'] == 2|| $pro['AVType'] == 6) ){
			$n_status = 1;
			$last_login = "NOW()";
