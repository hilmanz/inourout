...
class uploadHelper {
	function __construct($apps){
		...
		if(is_object($this->apps->user)) $this->uid = intval($this->apps->user->id);	
			...
			$this->typeImageAccepted = array("image/jpeg","image/jpg","image/png");
			...
			$this->extfileType = array("jpeg","jpg","png");
	...
	function profilePhoto(){
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			if ( isset($_FILES['profile_photo']) ) {
				if (! $_FILES['profile_photo']['error'] && $_FILES['profile_photo']['size'] < $max_file_size) {
					$ext = strtolower(pathinfo($_FILES['profile_photo']['name'], PATHINFO_EXTENSION));
					if (in_array($ext, $valid_exts)) {
							$filename = uniqid() . '.' . $ext;
							$path = $CONFIG['UPLOAD_ASSET'] . 'profile/' . $filename;
							$size = getimagesize($_FILES['profile_photo']['tmp_name']);
							...
							// pr($h);exit;
							// if($crop){
							//     if($ori_w < $nw or $ori_h < $nh) return "Picture is too small!";
							//     $ratio = max($nw/$ori_w, $nh/$ori_h);
							//     $h = $nh / $ratio;
							//     $x = ($ori_w - $nw / $ratio) / 2;
							//     $w = $nw / $ratio;
							//   }
							//   else{
							//     if($ori_w < $nw and $h < $nh) return "Picture is too small!";
							//     $ratio = min($nw/$ori_w, $nh/$ori_h);
							//     $w = $ori_w * $ratio;
							//     $h = $ori_h * $ratio;
							//     $x = 0;
							//   }
							...
							return array('result'=>false,'arrImage'=> "unknown problem!");
						} 
				} else {
					//echo 'file is too small or large';
					return array('result'=>false,'arrImage'=> "file is too small or large'");
				}
			} else {
				//echo 'file not set';
				return array('result'=>false,'arrImage'=> "file not set");
			}
		} else {
			//echo 'bad request!';
			return array('result'=>false,'arrImage'=> "bad request!");
		}
	}
	...
	function uploadThisImage($files=NULL,$path=NULL,$maxSize=1000,$resizeOriginal=false){
        ...
        if(in_array(strtolower($type[$jmlArr]),$this->extfileType)) {
