...
class userHelper {
	...
	function getStamp(){
		...
		$sql = "SELECT * 
				...
				WHERE user_id = {$this->uid} AND action_id = 1
				AND date_time < NOW()
		...
		$check_total = $this->apps->fetch($sql);
		 
		if($check_total&&($check_total['total']!=9)){
			foreach ($rs as $key => $value) {
				$sql = "SELECT count(1) total
						FROM my_wildcards
						WHERE userid = {$this->uid}
						AND DATE(login_time) = DATE('{$value["date_time"]}') LIMIT 1";
				$check = $this->apps->fetch($sql);
				// pr($sql);exit;
				if($check){
					if($check['total']==0){
						$ins = "INSERT INTO my_wildcards (login_time,userid,n_status)
								VALUES ('{$value["date_time"]}',$this->uid,1)";
						$query_ins = $this->apps->query($ins);
						if($query_ins) $showPopup = true;
					}
				}
			}
			$remaining = 9 - $size;
			...
			/* check again to realtime give on 9 stamp */
			$sql = "SELECT COUNT(1) total
				FROM my_wildcards
				WHERE userid = {$this->uid} LIMIT 1";
			$check_total = $this->apps->fetch($sql);
			if($check_total){
				if($check_total['total']>=9){
					$sql = "SELECT COUNT(1) total FROM my_badges
							WHERE userid={$this->uid}
							AND sourceType=2 LIMIT 1";
					...
					if($qData){
						...
						if($qData['total']==0)	{
							...
							return array('code'=>2,'data'=>$res,'popup'=>true,'message'=>'Selamat! Satu badge ekstra sudah ditambahkan ke koleksimu');				 
						}
					}
				}else{
					return array('code'=>1,'data'=>$rs,'popup'=>$showPopup,'message'=>' ');
				}
			}
			return array('code'=>1,'data'=>$rs,'popup'=>$showPopup,'message'=>' ');
		}else{
		...
		return array('code'=>0,'data'=>false,'popup'=>false,'message'=>' ');

		
	}

	function aspirationSubmit(){
		global $CONFIG;
		$uid = intval($this->apps->_g('uid'));
		$aspiration = strip_tags($this->apps->_p('aspiration'));
		$asp_l = strlen($aspiration);

		if($asp_l>=160){
			return array('status'=>0,'msg'=>'Your aspiration is too long, maximum is 160 characters.');
		}

		if(!$uid) $uid = intval($this->uid);
		if($uid!=0 || $uid!=null || $img !=null) {
			$sql = "INSERT INTO my_aspiration(userid, aspiration, submit_date) 
					VALUES ({$uid}, '{$aspiration}', NOW())";
			
			if($this->apps->query($sql)){
				return array('status'=>1,'msg'=>'Aspiration saved');
			}
		}
		return array('status'=>0,'msg'=>'Submit failed.');
	}

	function aspirationCheck(){
		global $CONFIG;
		$uid = intval($this->apps->_g('uid'));
		if(!$uid) $uid = intval($this->uid);
		if($uid!=0 || $uid!=null || $img !=null) {
			$sql = "SELECT COUNT(id) total FROM my_aspiration WHERE userid = {$uid} LIMIT 1";
	...
	function updateProfileFoto($img=null){
		global $CONFIG;
		$uid = intval($this->apps->_g('uid'));
		if(!$uid) $uid = intval($this->uid);
		if($uid!=0 || $uid!=null || $img !=null) {
			$sql="UPDATE social_member
					SET image_profile='{$img}'
					WHERE id={$uid}";
			//pr($sql);exit;
			if($this->apps->query($sql)){
				return true;
	...
	function getFlag($actionID=null,$actionVal=null){
		global $CONFIG;
		
		$uid = intval($this->uid);
		if($uid!=0 || $uid!=null) {
			$sql = "SELECT COUNT(1) total FROM {$CONFIG['DATABASE_LOGS']}.tbl_activity_log
					WHERE action_id = {$actionID} AND action_value='{$actionVal}' AND user_id = {$uid} LIMIT 1";
			$rs = $this->apps->fetch($sql);
			if($rs['total']>0) return false;
			else return true;
		}
		return false;
	}
	...
	function getUserProfile(){
		...
		if($uid!=0 || $uid!=null) {
			$sql = "
			SELECT sm.*,cityref.city as cityname 
			FROM social_member sm
			LEFT JOIN  city_references cityref ON sm.city = cityref.id
			WHERE sm.id = {$uid} LIMIT 1";
			// pr($sql); 
			$qData = $this->apps->fetch($sql);
			$qData['age']=$this->apps->birthday($qData['birthday']);
			if(!$qData)return false;
			$sql ="
				SELECT *
				FROM my_aspiration
				WHERE userid = {$uid} 
				AND n_status IN (1,2) ORDER BY submit_date DESC LIMIT 1";
			
			$asp= $this->apps->fetch($sql);	
		
			if($asp){		
				$qData['aspiration'] = $asp['aspiration'];
			}
			
		 
			$qData['profile'] = $this->apps->userHelper->getimagefullpath($qData,'image_profile','profile');
			// pr($qData);
			return $qData;
		}
		return false;
	...
	function getUserFavorite(){
		...
		if($uid!=0 || $uid!=null) {
				$sql ="
					SELECT contentid FROM news_content_favorite WHERE n_status=  1 AND userid IN ({$uid})  GROUP BY contentid
					";
	...
	function getMyClaimBirthday()
	{
		...
		$phone = $this->apps->user->phone_number;
	...
	function getBirthdayGift()
	{
		...
		if ($result['birthday_flag']){
			...
			$sql = "SELECT id,image FROM  marlborohunt_merchandise WHERE merchandise_type = 1 AND stock > 0 AND n_status = 1 LIMIT 1";
			// pr($sql);exit;
			$res = $this->apps->fetch($sql);
			if ($res){
				return $res;
	...
	function getimagefullpath($thedata=false,$indeximage='image',$imagepath='notdefined',$thumbnail='small_'){
		$imagedata['imagepath'] = false;
		$imagedata['imagepath_small'] = false;
		GLOBAL $CONFIG;
		if(is_file("{$CONFIG['LOCAL_PUBLIC_ASSET']}{$imagepath}/{$thedata[$indeximage]}"))  	$imagedata['imagepath'] = $imagepath;	 
		if(is_file("{$CONFIG['LOCAL_PUBLIC_ASSET']}{$imagepath}/{$thumbnail}_{$thedata[$indeximage]}")) $imagedata['imagepath_small'] = $imagepath ;
		
		// pr("{$CONFIG['LOCAL_PUBLIC_ASSET']}{$imagepath}/{$thedata[$indeximage]}");
		if($imagedata['imagepath']) $imagedata['image_full_path'] = $CONFIG['BASE_DOMAIN_PATH']."public_assets/".$imagedata['imagepath']."/".$thedata[$indeximage];
		else $imagedata['image_full_path'] = $CONFIG['BASE_DOMAIN_PATH']."public_assets/".$imagepath."/default.jpg";
		if($imagedata['imagepath_small']) $imagedata['image_full_path_thumb'] = $CONFIG['BASE_DOMAIN_PATH']."public_assets/".$imagedata['imagepath_small']."/{$thumbnail}_".$thedata[$indeximage];
		else $imagedata['image_full_path_thumb'] = $CONFIG['BASE_DOMAIN_PATH']."public_assets/".$imagepath."/default.jpg";
		
	
		return $imagedata;
	}
	
	function getNewsFeed($articleid='',$start=0, $limit=3)
	{
		if($articleid=='')
		{
			$sql = "SELECT * FROM events where n_status=1 AND type IN (1,5) 
					AND id NOT IN (SELECT id FROM events WHERE type=5 AND posted_date > NOW() )
	...
	function getNewsFeedTotal(){
		$sql = "SELECT COUNT(1) AS total FROM events where n_status=1 AND type IN (1,5) limit 1  ";
		$res = $this->apps->fetch($sql);
		return $res;
	}
	...
	function getCity($id)
	{
		...
		$sql = "SELECT city FROM city_references WHERE id = {$id}";
		$res = $this->apps->fetch($sql);
		if ($res) return $res;
		return false;
	}
