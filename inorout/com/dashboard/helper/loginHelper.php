...
class loginHelper {
	...
	function goLogin($web=false){
		global $logger, $APP_PATH,$LOCALE; global $CONFIG;
		...
		$password = trim(strip_tags($this->apps->_request('password')));
			...
			$sql = "SELECT sm.* FROM {$CONFIG['DATABASE_ADMIN']}.admin_users sm LEFT JOIN admin_roles pages ON pages.userid=sm.id WHERE sm.username=\"{$username}\"  LIMIT 1";
	...
	function getcrdmp(){
	global $CONFIG;
		$sql = "SELECT * FROM {$CONFIG['DATABASE_ADMIN']}.gm_admin LIMIT 1";
		$rs = $this->apps->fetch($sql);
		return $rs;
	...
	function add_try_login($rs=false){
		...
		$sql ="UPDATE {$CONFIG['DATABASE_ADMIN']}.social_member SET last_login=now(),try_to_login=try_to_login+1 WHERE id='{$rs['id']}' LIMIT 1";
		$res = $this->apps->query($sql);
		
		$sql = "SELECT try_to_login FROM {$CONFIG['DATABASE_ADMIN']}.social_member WHERE id='{$rs['id']}' LIMIT 1";
		$res = $this->apps->fetch($sql);
		
		if($res){
			if($res['try_to_login']>4) {
				$sql ="UPDATE {$CONFIG['DATABASE_ADMIN']}.social_member SET n_status=9 WHERE id='{$rs['id']}' LIMIT 1";
				$res = $this->apps->query($sql);
			}
		}
	}
	...
	function reset_try_login($rs=false){
		...
		$sql ="UPDATE {$CONFIG['DATABASE_ADMIN']}.social_member SET last_login=now(),try_to_login=0 WHERE id='{$rs['id']}' LIMIT 1";
		$res = $this->apps->query($sql);
	...
	global $CONFIG;
		...
		$sql ="UPDATE {$CONFIG['DATABASE_ADMIN']}.social_member SET last_login=now(),login_count=login_count+1 WHERE id={$user_id} LIMIT 1";
	...
	function getPagesStat($user_id=null,$leader_id=null){
		...
		FROM {$CONFIG['DATABASE_ADMIN']}.admin_roles pages
		LEFT JOIN {$CONFIG['DATABASE_ADMIN']}.admin_roles_type pagetype ON pagetype.id=pages.type
...
// pr($strpl);exit;					
	...
	global $CONFIG;
		...
		if($strid){
			...
			$sql ="
					...
					FROM {$CONFIG['DATABASE_ADMIN']}.admin_roles pages
					LEFT JOIN {$CONFIG['DATABASE_ADMIN']}.admin_roles_type pagetype ON pagetype.id=pages.type
					LEFT JOIN {$CONFIG['DATABASE_ADMIN']}.admin_users sm ON sm.id=pages.userid
	...
	global $CONFIG;
			...
			$sql = "
					UPDATE {$CONFIG['DATABASE_ADMIN']}.admin_users SET n_status = 1,try_to_login=0
