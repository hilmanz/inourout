...
class newsHelper{
	...
	function newsList($start=null,$limit=10)
	{
		...
		$sql = "
			...
			WHERE type IN(1,5) AND n_status = 1 {$filter} ORDER BY created_date DESC,id DESC LIMIT {$start},{$limit}
		";
		// pr($sql);
	...
	function insertnewsupdate()
	{
		...
		$posted_date = $this->apps->_p('posted_date');
		...
		if(isset($submit)){
			$sql = "INSERT INTO {$CONFIG['DATABASE_WEB']}.events (`title`, `brief`, `content`, `type`, `created_date`, `unpublished_date`, `posted_date`, `cityid`, `n_status`) 
					VALUES ('{$title}', '{$brief}', '{$content}', '{$type}', '{$created_date}', '{$unpublished_date}', '{$posted_date}', '{$city}', '{$n_status}')";
			// pr($sql);exit;
	...
	{	
		global $CONFIG;
		$sql = "SELECT * FROM {$CONFIG['DATABASE_WEB']}.city_references WHERE usedonweb = 1";
