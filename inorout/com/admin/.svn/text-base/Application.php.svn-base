...
class Application extends SQLData{
	...
	function isUserOnline(){
		global $LOCALE;
		if($this->session->getSession($this->config['SESSION_NAME'],"admin")) return true;
		return false;
		
	}
	
	function getUserOnline(){
		if($this->session->getSession($this->config['SESSION_NAME'],"admin")) return $this->session->getSession($this->config['SESSION_NAME'],"admin");
		else return false;
